...
import CorrectionColar from "./CorrectionColar.vue";
import FiltrationModal from "./FiltrationModal.vue";

export default defineComponent({
  name: "MainCanvas",
  components: { CorrectionColar, FiltrationModal },
  props: {
    state: String,
    img: String,
    origImg: Object,
  ...
  methods: {
    ...
    renderImage(newImg) {
      ...
      img.onload = () => {
        ...
        const [dx, dy, iw, ih] = this.calculateImageSizes(canvas, img);
    ...
    onMouseDown(e) {
      ...
      if (this.state == "pipette") {
        this.$emit("updateColor", this.pickColor(e));
        this.$emit("updateCoordinates", this.getCoordinates(e));
      }
    },
    onMouseUp() {
    ...
    calculateImageSizes(canvas, img) {
      const offset = 100;
    ...
    getCoordinates({ offsetX, offsetY, clientX, clientY }) {
